...
node("Node1") {
   stage('Preparation (Checking out)') {
       try {
           checkout([$class: 'GitSCM', branches: [[name: '*/ypapkou']], doGenerateSubmoduleConfigurations: false, extensions: [], submoduleCfg: [], userRemoteConfigs: [[url: 'https://github.com/repo_url/']]])
       }
       catch (exc) {
           mail bcc: '', body: "\"${STAGE_NAME}\" stage is failed. Build number is $BUILD_NUMBER.", cc: '', from: '', replyTo: '', subject: "INFO:$STAGE_NAME, BUILD:$BUILD_NUMBER", to: 'mail@gmail.com'
           throw exc
       }
   ...
   stage('Triggering job and fetching artefact after finishing') {
       try {
           build job: 'ypapkou-child1-build-job', parameters: [string(name: 'BRANCH_NAME', value: "$student")]
           copyArtifacts filter: '${student}_dsl_script.tar.gz, output.txt', fingerprintArtifacts: true, projectName: 'ypapkou-child1-build-job', selector: lastSuccessful()
       }
       catch (exc) {
           mail bcc: '', body: "\"${STAGE_NAME}\" stage is failed. Build number is $BUILD_NUMBER.", cc: '', from: '', replyTo: '', subject: "INFO:$STAGE_NAME, BUILD:$BUILD_NUMBER", to: 'mail@gmail.com'
           throw exc
   ...
   stage('Deployment') {
       try {
           ...
           mail bcc: '', body: "\"${STAGE_NAME}\" stage has passed. Build number is $BUILD_NUMBER.", cc: '', from: '', replyTo: '', subject: "ATTENTION:$STAGE_NAME, BUILD:$BUILD_NUMBER", to: 'mail@gmail.com'
       }
       catch (exc) {
           mail bcc: '', body: "\"${STAGE_NAME}\" stage is failed. Build number is $BUILD_NUMBER.", cc: '', from: '', replyTo: '', subject: "WARNING:$STAGE_NAME, BUILD:$BUILD_NUMBER", to: 'mail@gmail.com'
           throw exc
       }
   }
}
